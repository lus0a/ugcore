...
/*
			//Shuai
			typedef typename grid_dim_traits<domain_type::dim>::grid_base_object elem_type;
			elem_type* Ele = (elem_type *) elem;
			std::vector<int> ind_check;
			dd->indices(Ele->vertex(0), ind, Eval.use_hanging());
			ind_check.push_back(ind.index(0,0));
			dd->indices(Ele->vertex(1), ind, Eval.use_hanging());
			ind_check.push_back(ind.index(0,0));
			dd->indices(Ele->vertex(2), ind, Eval.use_hanging());
			ind_check.push_back(ind.index(0,0));
			dd->indices(Ele->vertex(3), ind, Eval.use_hanging());
			ind_check.push_back(ind.index(0,0));
*/
		//	get global indices
...
/*			for (size_t i=0; i<4; i++)
			{
				if (ind_check[i]!=ind.index(0,i))
				{
					UG_THROW ("elem_disc_assemble_util.h: AssembleDefect: Two inds are not equal at Vertex'" << i );
				}
			}
*/			
...
/*		//Shuai
			for (size_t i=0; i<4; i++)
			{
				if (ind_check[i]!=ind.index(0,i))
				{
					UG_THROW ("elem_disc_assemble_util.h: Two inds are not equal at Vertex'" << i );
				}
			}
*/
